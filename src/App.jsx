...
// ââ XIRR (Money-Weighted Return) âââââââââââââââââââââââââââââââââââââââââââââ
...
function EvoMini({en,trades,fxRate,liveT10Y,liveFX,liveSP500,historicos,isModal=false,livePricesAll={},onExpand=null}){
  ...
  const xirrData=useMemo(()=>{
    ...
    try{
      ...
      // Detectar bonos aunque ya no estén en cartera
      ...
      // Buscar precio histórico (último <= date). Si no hay, usar el primero >= date. Si no hay, 0.
      const _findHistPrice=(ticker,dateStr)=>{
        const bars=(historicos?.[ticker]||[]);
        if(!bars.length) return 0;
        // asume bars ordenados por date asc (en tu JSON lo están). Igual por las dudas:
        // NO hacemos sort por performance; tomamos como vienen.
        let lo=0,hi=bars.length-1,res=-1;
        while(lo<=hi){const mid=(lo+hi)>>1; if(bars[mid].date<=dateStr){res=mid;lo=mid+1;}else hi=mid-1;}
        if(res>=0) return bars[res].close||0;
        // no hay <= date → tomar el primero >= date
        for(const b of bars){ if(b.date>=dateStr) return b.close||0; }
        return 0;
      };

      // Posición neta a una fecha
      ...
      // Valor USD de una posición a una fecha (con fallback a precio de compra)
      const valuePosUSD = (posMap, dateStr) => {
        ...
        for(const [tkr,qty] of Object.entries(posMap||{})){
          ...
          if(!price||price<=0){
            const lastBuy=(trades||[]).filter(t=>String(t.ticker||'').toUpperCase()===T && t.tipo==="compra" && t.date<=dateStr)
      ...
      // Construir flujos del período
      const includeStartDayAsPosition = (s === (trades||[]).map(t=>t?.date).filter(Boolean).sort()[0]);
      ...
      // Si endValUSD no sale por histórico (falta barra exacta), usar valor en vivo como proxy
      const endValNow = en.reduce((a,h)=>a+h.valUSD,0);
      if(!endValUSD || endValUSD<=0) endValUSD = endValNow;

      // Trades dentro del periodo (excluir día final e para evitar doble conteo del terminal)
      ...
      let portXIRR = (flows.length>=2 && startValUSD>0 && endValUSD>0) ? calcXIRR(flows) : null;

      // SPY XIRR (mismos cashflows, pero terminal = cada flujo crecido por SPY hasta e)
      let spyXIRR=null;
      if(cd.spy100 && cd.spy100.length>=2){
        ...
        if(spyEnd>0){
          ...
          for(const fl of flows.slice(1, -1)) spyFlows.push(fl);
          // terminal
          let spyFinalVal=0;
          ...
          spyXIRR = (spyFlows.length>=2 && startValUSD>0 && spyFinalVal>0) ? calcXIRR(spyFlows) : null;
        }
      }
    ...
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <span style={{fontSize:window.innerWidth<768?14:10,color:"var(--green)"}}>â Portfolio</span>
      ...
      {xirrData&&(()=>{
        ...
        const fXP=v=>v!=null?(v>=0?"+":"")+v.toFixed(1)+"%":"â";
