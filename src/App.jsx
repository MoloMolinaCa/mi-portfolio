...
function CountdownDisplay({lastRefresh, priceStatus, liveCount, portLen}){
  ...
  if(priceStatus==="loading") return <span style={{color:"var(--text-muted)"}}>âŸ³ Actualizando...</span>;
  return null;
}
...
const ASSET_TYPES = {
  accion_ar: { label: "Acciones AR",  color: "#3B82F6", icon: "ðŸ“ˆ" },
  cedear:    { label: "CEDEARs",      color: "#10B981", icon: "ðŸŒŽ" },
  bono_ars:  { label: "Bonos ARS",    color: "#F59E0B", icon: "ðŸ“œ" },
  bono_usd:  { label: "Bonos USD",    color: "#F97316", icon: "ðŸ’µ" },
  fci_ars:   { label: "FCI Pesos",    color: "#8B5CF6", icon: "ðŸ¦" },
  fci_usd:   { label: "FCI DÃ³lares",  color: "#A78BFA", icon: "ðŸ’°" },
};
...
// Ãšltimo dÃ­a hÃ¡bil (lunes-viernes) <= fecha dada
...
function calcTWR(dates, trades, en, tickerBars, cclBars, mepBars, currency, fxRate, livePricesMap, customEnd=null, realTodayStr=null){
  ...
  // Ãšltimo precio disponible <= fecha (bisecciÃ³n O(log n))
...
function EvoMini({en,trades,fxRate,liveT10Y,liveFX,liveSP500,historicos,isModal=false,livePricesAll={},onExpand=null}){
  ...
  const findPrice=(bars,dateStr)=>{
    ...
    // Ãšltimo precio <= fecha (nunca usar precio futuro)
    let lo=0,hi=bars.length-1,res=-1;
    while(lo<=hi){
      const mid=(lo+hi)>>1;
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      ...
      <div style={{flex:1,overflow:"visible",minHeight:0,position:"relative"}}>
        {loading&&<div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text-muted)",fontSize:12}}><span style={{animation:"spin 0.8s linear infinite",display:"inline-block",marginRight:6}}>âŸ³</span>Cargando...</div>}
  ...
  return(
    <div>
      ...
      <div style={{position:"relative"}}>
        ...
        />
        {value&&<span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:13}}>âœ…</span>}
        ...
        {step==='confirm' && seedFlows && (
          <>
                      ...
                      <tr key={i} style={{borderBottom:"1px solid var(--border)"}}>
                        ...
                        <td style={{padding:"6px 10px"}}>
                          <span style={{color:f.tipo==='amortizacion'?"var(--yellow)":"var(--accent)",fontWeight:600}}>
                            {f.tipo==='amortizacion'?'ðŸ’° Amort.':'ðŸŽ« CupÃ³n'}
                          </span>
                        </td>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              ...
              <button onClick={()=>handleConfirm([...seedFlows])} style={btn("var(--accent)")}>âœ“ Confirmar y cargar</button>
            </div>
          </>
        )}
          ...
          <>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              ...
              <button onClick={handleGenerate} disabled={!params.vto||!params.tna||!params.primerCupon||generating}
                ...
                {generating ? <><span style={{animation:"spin 0.7s linear infinite",display:"inline-block"}}>âŸ³</span> Cargando feriados...</> : "Generar flujos â†’"}
              </button>
            </div>
          </>
        )}
          ...
          <>
            {!amortOk&&(
              <div style={{background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:7,padding:"7px 12px",marginBottom:10,fontSize:12,color:"var(--red)"}}>
                âš  AmortizaciÃ³n total: <b>{totalAmort.toFixed(4)}%</b> â€” debe ser exactamente 100%
              </div>
            )}
            {amortOk&&(
            ...
            {params.tna&&(
              <div style={{marginBottom:10,display:"flex",justifyContent:"flex-end"}}>
                <button onClick={recalcularTasas} disabled={recalculating}
                  ...
                  {recalculating?<><span style={{animation:"spin 0.7s linear infinite",display:"inline-block"}}>âŸ³</span>Calculando...</>:<>ðŸ”„ Recalcular tasas <span style={{fontWeight:400,opacity:0.7}}>({params.tna}% TNA)</span></>}
                </button>
              </div>
            )}
                    ...
                    <tr key={i} style={{borderBottom:"1px solid rgba(255,255,255,0.04)",background:row.amort>0?"rgba(251,191,36,0.03)":"transparent"}}>
                      ...
                      <td style={{padding:"4px 2px",textAlign:"center"}}>
                        <button onClick={()=>setEditRows(editRows.filter((_,j)=>j!==i))}
                          style={{background:"transparent",border:"none",color:"var(--text-muted)",cursor:"pointer",fontSize:13,padding:"2px 3px"}}>ðŸ—‘</button>
                      </td>
                    </tr>
                    );
                  })}
      ...
      <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:16,padding:28,width:520,maxWidth:"95vw",maxHeight:"90vh",overflowY:"auto"}}>
        ...
        <div style={{display:"grid",gap:14}}>
          ...
          <div>
            ...
            {f.operacion==="venta"&&<div style={{fontSize:11,color:"var(--yellow)",marginTop:5}}>âš  FIFO â€” salen los lotes mÃ¡s antiguos primero</div>}
          </div>
            ...
            <div>
              ...
              <div style={{position:"relative",marginTop:4}}>
                ...
                <span style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",fontSize:14}}>
                  {tickerStatus==="checking"&&<span style={{animation:"spin 0.8s linear infinite",display:"inline-block"}}>âŸ³</span>}
                  {tickerStatus==="confirmed"&&"âœ…"}
                    ...
                    {searchResults.map((r,i)=>(
                      <div key={i} onClick={()=>selectResult(r)}
                        ...
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{fontSize:10,color:"var(--text-muted)",marginTop:1}}>ðŸ“¡ {r.source}</div>
                        </div>
              ...
              {selectedResult&&(
                <div style={{marginTop:8,background:"rgba(52,211,153,0.07)",border:"1px solid rgba(52,211,153,0.3)",borderRadius:8,padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    ...
                    <div style={{fontSize:10,color:"var(--text-muted)",marginTop:2}}>ðŸ“¡ {selectedResult.source} Â· {selectedResult.buyCurrency}</div>
                  </div>
              ...
              {tickerStatus==="notfound"&&<div style={{marginTop:8,background:"rgba(251,191,36,0.07)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:8,padding:"10px 12px",fontSize:12,color:"var(--yellow)"}}>âš ï¸ Sin resultados en data912 ni Yahoo â€” podÃ©s guardar igual ingresando los datos manualmente.</div>}
            </div>
          )}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:12}}>
            <label style={{display:"flex",flexDirection:"column",gap:4}}>
              ...
              {f.operacion==="venta"&&f.ticker&&<div style={{fontSize:10,color:overSelling?"var(--red)":"var(--text-muted)",marginTop:3}}>
                {overSelling?`âš  Solo tenÃ©s ${availableQty.toLocaleString("es-AR")} nominales`:`Disponible: ${availableQty.toLocaleString("es-AR")} nominales`}
              </div>}
            </label>
        ...
        <div style={{display:"flex",gap:10,marginTop:22,justifyContent:"space-between",alignItems:"center"}}>
          {h&&<button onClick={()=>{if(window.confirm("Â¿Eliminar esta posiciÃ³n?"))onSave(null);}} style={{padding:"8px 14px",background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:8,color:"var(--red)",cursor:"pointer",fontSize:12}}>ðŸ—‘ Eliminar</button>}
          {!h&&<div/>}
        ...
        <div style={{height:200,position:"relative"}}>
          {loading&&(
            <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",color:"var(--text-muted)",fontSize:13}}>
              <span style={{animation:"spin 0.8s linear infinite",display:"inline-block",marginRight:8,fontSize:18}}>âŸ³</span>
                    ...
                    <td style={{padding:"8px",textAlign:"right"}}>
                      {isEditing?(
                        <div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
                          <button onClick={saveEdit} style={{padding:"4px 10px",background:"var(--green)",border:"none",borderRadius:5,color:"#fff",cursor:"pointer",fontSize:11,fontWeight:600}}>âœ“</button>
                          <button onClick={()=>{setEditId(null);setEditData(null);}} style={{padding:"4px 8px",background:"var(--bg-input)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text-muted)",cursor:"pointer",fontSize:11}}>âœ•</button>
                        </div>
                      ):(
                        <div style={{display:"flex",gap:4,justifyContent:"flex-end"}}>
                          <button onClick={()=>startEdit(t)} style={{padding:"4px 8px",background:"var(--bg-input)",border:"1px solid var(--border)",borderRadius:5,color:"var(--text-muted)",cursor:"pointer",fontSize:11}}>âœï¸</button>
                          <button onClick={()=>setConfirmDelete(t)} style={{padding:"4px 8px",background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:5,color:"var(--red)",cursor:"pointer",fontSize:11}}>ðŸ—‘</button>
                        </div>
                      )}
                    </td>
                  </tr>
      ...
      {confirmDelete&&(
        <div className={darkMode?"theme-dark":"theme-light"} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:300}}>
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:16,padding:28,width:420,maxWidth:"95vw"}}>
            ...
            <div style={{fontSize:12,color:"var(--yellow)",marginBottom:20}}>âš  Esta acciÃ³n elimina la operaciÃ³n del historial. Si era la Ãºnica compra del activo, se elimina del portfolio.</div>
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
  ...
  return(
    <div className="fi" style={{display:"grid",gap:isMobile?10:16}}>
      ...
      {(!isMobile||mobileSection==="extremos")&&<div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:12}}>
        {[
          {label:"ðŸ“Š Mi Portfolio", ex:portExtremes},
          {label:"ðŸ‡ºðŸ‡¸ S&P 500",      ex:spExtremes},
        ].map(({label,ex})=>(
            ...
            {ex ? (
              <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"1fr 1fr",gap:10}}>
                ...
                <div style={{background:"rgba(248,113,113,0.07)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:8,padding:"12px 14px"}}>
                  ...
                  {ex.recDays!=null
                    ? <div style={{fontSize:10,color:"var(--green)",marginTop:3}}>âœ“ RecuperÃ³ en {ex.recDays} dÃ­as</div>
      ...
      {(!isMobile||mobileSection==="correlacion")&&<div style={{...card,padding:isMobile?"10px 12px":"16px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:isMobile?"flex-start":"center",flexDirection:isMobile?"column":"row",gap:isMobile?6:0,marginBottom:12}}>
          ...
          <span style={{fontSize:10,color:"var(--text-muted)",fontStyle:"italic",background:"rgba(251,191,36,0.08)",border:"1px solid rgba(251,191,36,0.2)",borderRadius:5,padding:"2px 8px"}}>âš  Usa todo el histÃ³rico Â· no aplica filtro</span>
        </div>
        {(()=>{
          ...
          // Ïƒ_pÂ² = Î£_i Î£_j w_i * w_j * Ïƒ_i * Ïƒ_j * Ï_ij
          let varPort = 0;
          ...
          // MCTR_i = w_i * Î£_j (w_j * Ïƒ_i * Ïƒ_j * Ï_ij) / Ïƒ_p
                ...
                <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:isMobile?6:8}}>
                  {(()=>{
                    ...
                    const volMsg   = volPort<8
                      ? "âœ“ Volatilidad baja â€” cartera conservadora"
                      : volPort<15
                      ? "âš  Volatilidad moderada â€” nivel aceptable pero atenciÃ³n"
                      : "âœ— Volatilidad alta â€” la cartera oscila demasiado";
                    const ratioColor = divRatio>1.5?"var(--green)":divRatio>1.1?"var(--yellow)":"var(--red)";
                    const ratioMsg   = divRatio>1.5
                      ? "âœ“ Buena diversificaciÃ³n â€” los activos se compensan bien"
                      : divRatio>1.1
                      ? "âš  DiversificaciÃ³n moderada â€” podrÃ­as mejorarla"
                      : "âœ— Poca diversificaciÃ³n â€” los activos se mueven muy juntos";
                    ...
                    const nEffMsg   = nEffNum/activos.length>0.6
                      ? "âœ“ Buena variedad â€” tus activos son realmente distintos"
                      : nEffNum/activos.length>0.4
                      ? "âš  Variedad moderada â€” algunos activos se solapan"
                      : "âœ— Poca variedad real â€” varios activos se mueven igual";
                    const kpis = [
                ...
                <div>
                  ...
                  {totalMesUSD>0&&(
                    <div style={{fontSize:11,color:'var(--accent)',marginTop:2}}>
                      Total del mes: <b style={{fontFamily:"'DM Mono',monospace"}}>â‰ˆ US${fmtN2(totalMesUSD)}</b>
                    </div>
                  )}
                </div>
                ...
                return(<>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12}}>
                    ...
                    <button onClick={()=>setCalSelectedDate(null)}
                      style={{background:'transparent',border:'none',color:'var(--text-muted)',cursor:'pointer',fontSize:16}}>âœ•</button>
                  </div>
                      ...
                      <div key={p.ticker+p.date} onClick={()=>setSelected(p.ticker)}
                        ...
                        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:6}}>
                          ...
                          <span style={{fontSize:10,color:p.hasAmort?'var(--yellow)':'var(--accent)'}}>
                            {p.hasAmort&&p.hasCupon?'ðŸ’°+ðŸŽ«':p.hasAmort?'ðŸ’°':'ðŸŽ«'}
                          </span>
                        </div>
          ...
          {bonds.map(b=>{
            ...
            return(
              <button key={b.ticker}
                ...
                {loadingTicker===b.ticker?'âŸ³ ':hasFlows?'âœ“ ':'+ '}
                {b.ticker}
              </button>
            );
          })}
          ...
          <div>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                ...
                {editingMeta ? (
                  ...
                  <div style={{display:'flex',gap:6,alignItems:'center',background:'var(--bg-input)',border:'1px solid var(--border)',borderRadius:8,padding:'6px 12px',cursor:'pointer'}}
                    ...
                    <span style={{fontSize:10,color:'var(--text-muted)',marginLeft:4}}>âœï¸</span>
                  </div>
                )}
            ...
            {adjustsBy==='CER'&&(()=>{
              ...
              return(
                <div style={{background:'rgba(251,191,36,0.06)',border:'1px solid rgba(251,191,36,0.2)',borderRadius:8,padding:'8px 14px',marginBottom:10,display:'flex',alignItems:'center',gap:16,flexWrap:'wrap',fontSize:11}}>
                  <span style={{color:'var(--yellow)',fontWeight:600}}>âš¡ CER</span>
                  ...
                  {cerSerie&&!emisionDate&&<span style={{color:'var(--text-muted)'}}>EditÃ¡ el chip âœï¸ para ingresar la fecha de emisiÃ³n</span>}
                  ...
                  {cerSerie&&cerSerie.length>0&&!cerLoading&&<span style={{color:'var(--green)',marginLeft:'auto',fontSize:10}}>âœ“ {cerSerie.length} registros</span>}
            ...
            {addingFlow===selected&&(
              <div style={{background:editingRowIds?'rgba(96,165,250,0.06)':'rgba(59,130,246,0.06)',border:`1px solid ${editingRowIds?'rgba(96,165,250,0.3)':'rgba(59,130,246,0.15)'}`,borderRadius:10,padding:'14px 16px',marginBottom:12,display:'flex',gap:10,alignItems:'flex-end',flexWrap:'wrap'}}>
                {editingRowIds&&<div style={{width:'100%',fontSize:11,color:'#60A5FA',fontWeight:600,marginBottom:2}}>âœï¸ Editando fila â€” modificÃ¡ los campos y guardÃ¡</div>}
                {/* F. Pago */}
                ...
                <div style={{display:'flex',flexDirection:'column',gap:4,minWidth:140}}>
                  ...
                  <select value={newFlow.tipo} onChange={e=>setNewFlow(p=>({...p,tipo:e.target.value}))} style={inp}>
                    <option value="cupon">ðŸŽ« Solo cupÃ³n</option>
                    <option value="amortizacion">ðŸ’° Solo amort.</option>
                    <option value="ambos">ðŸ’°+ðŸŽ« Amort.+CupÃ³n</option>
                  </select>
                </div>
                {/* Amort */}
                ...
                <button onClick={editingRowIds ? saveEditFlow : addNewFlow}
                  ...
                  {editingRowIds ? 'âœ“ Guardar cambios' : 'Guardar'}
                </button>
            ...
            {rows.length===0
              ...
              : (<>
                ...
                <div style={{display:'flex',gap:8,marginBottom:12}}>
                  {[['prospecto','ðŸ“‹ Prospecto'],['micobro','ðŸ’° Mi cobro']].map(([k,lbl])=>(
                ...
                {viewMode==='prospecto'&&(()=>{
                  ...
                  <div style={{overflowX:'auto',borderRadius:10,border:'1px solid var(--border)'}}>
                          ...
                          return(
                            <tr key={row.date} style={{background:rowBg}}>
                              ...
                              <td style={{...tdPL,fontWeight:600,color:isCobrado?'var(--green)':isPast?'var(--yellow)':'var(--text-primary)'}}>
                                {fmtD(row.date)}{isCobrado&&<span style={{fontSize:9,marginLeft:5,color:'var(--green)'}}>âœ“</span>}
                              </td>
                              ...
                              <td style={{...tdP,textAlign:'center',whiteSpace:'nowrap'}}>
                                <button onClick={()=>{
                                  ...
                                  title="Editar" style={{background:'transparent',border:'none',cursor:'pointer',color:'#60A5FA',fontSize:11,padding:'0 2px'}}>âœï¸</button>
                                <button onClick={()=>deleteRow(selected,row.ids)}
                                  title="Eliminar" style={{background:'transparent',border:'none',cursor:'pointer',color:'var(--red)',fontSize:12,padding:'0 2px'}}>ðŸ—‘</button>
                              </td>
                            </tr>
                          );
                        })}
                    ...
                    <div style={{fontSize:9,color:'var(--text-muted)',padding:'6px 10px'}}>
                      âœŽ Click en celda para editar
                  ...
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
                    ...
                    <tbody>
                      {rows.map(row=>{
                        ...
                        const tipoLabel = hasAmort&&hasCupon?'ðŸ’°+ðŸŽ« Amort.+CupÃ³n':hasAmort?'ðŸ’° Amort.':'ðŸŽ« CupÃ³n';
                        ...
                        return(
                          <tr key={row.date} style={{borderBottom:'1px solid var(--border)',
                            ...
                            <td style={{padding:'10px 12px',fontSize:12}}>
                              {cobrado
                                ?<span style={{color:'var(--green)'}}>âœ… {fmtD(row.cupon?.fechaCobro||row.amort?.fechaCobro)}</span>
                                :!isFuture
                                  ...
                                  :<span style={{color:'var(--text-muted)',fontSize:11}}>ðŸŸ¡ Pendiente</span>
                              }
                            </td>
                            <td style={{padding:'10px 12px'}}>
                              <button onClick={()=>deleteRow(selected,row.ids)}
                                style={{background:'transparent',border:'none',cursor:'pointer',color:'var(--red)',fontSize:13}}>ðŸ—‘</button>
                            </td>
                          </tr>
                        );
                      })}
    ...
    <>
      ...
      <div style={{minHeight:"100vh",background:"var(--bg)",color:"var(--text-primary)",overflowX:"hidden",maxWidth:"100vw"}} className={darkMode?"theme-dark":"theme-light"}>
        ...
        <div style={{background:"var(--bg-card)",borderBottom:"1px solid var(--border)",padding:isMobile?"8px 12px":"11px 24px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:10,minWidth:0}}>
              <div style={{width:28,height:28,flexShrink:0,borderRadius:8,background:"linear-gradient(135deg,var(--accent),var(--accent2))",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14}}>ðŸ“Š</div>
              <div style={{minWidth:0}}>
                ...
                {!isMobile&&<div style={{fontSize:11,color:"var(--text-muted)",display:"flex",alignItems:"center",gap:8}}>
                  ...
                  {syncStatus==="error"&&<span style={{color:"var(--red)"}}>âš  error de sync</span>}
            ...
            <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
              {!isMobile&&<div style={{display:"flex",flexDirection:"column",gap:1}}>
                <select value={fx} onChange={e=>setFx(e.target.value)} style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:6,padding:"5px 10px",color:"var(--text-secondary)",fontSize:12,cursor:"pointer"}}>
                  <option value="CCL">ðŸ’µ CCL {fmtA(liveFX.CCL)}</option>
                  <option value="MEP">ðŸ’µ MEP {fmtA(liveFX.MEP)}</option>
                  <option value="oficial">ðŸ’µ Oficial {fmtA(liveFX.oficial)}</option>
                </select>
          ...
          {isMobile&&<div style={{marginTop:6,display:"flex",gap:8,alignItems:"center",justifyContent:"space-between"}}>
            ...
            <span style={{fontSize:10,display:"flex",alignItems:"center",gap:6}}>
              ...
              {syncStatus==="error"&&<span style={{color:"var(--red)",fontSize:9}}>âš  sync</span>}
        ...
        <div style={{background:"var(--bg-card)",borderBottom:"1px solid var(--border)",padding:"0 12px",display:"flex",gap:0,overflowX:"auto",WebkitOverflowScrolling:"touch",scrollbarWidth:"none"}}>
          {[["dashboard","ðŸ“Š Dashboard"],["portfolio","ðŸ’¼ Portfolio"],["analisis","ðŸ” AnÃ¡lisis"],["flujos","ðŸ’¸ Flujos"],["operaciones","ðŸ“‹ Operaciones"]].map(([id,lbl])=>(
            ...
            <div className="fi" style={{display:"grid",gap:16}}>
              ...
              {(()=>{
                ...
                const kpis=[
                  ...
                  {
                    icon:"ðŸ“…", lbl:"Rendimiento del dÃ­a",
                    main:fmtP(dayPct),
                    sub:hideAmounts?"â€¢â€¢â€¢â€¢":(dayPnlUSD>=0?"+":"")+fmtU(dayPnlUSD)+(spyDayPct!=null?" Â· S&P "+(spyDayPct>=0?"+":"")+spyDayPct.toFixed(2)+"%":""),
              ...
              return(
                <div style={{background:"var(--bg-input)",borderRadius:12,padding:"14px 16px"}}>
                  <div style={{fontSize:11,color:"var(--text-muted)",textTransform:"uppercase",letterSpacing:1,marginBottom:12,display:"flex",justifyContent:"space-between"}}>
                    ...
                    {loading&&<span style={{animation:"spin 0.8s linear infinite",display:"inline-block"}}>âŸ³</span>}
                  </div>
                  {benchmarks.map(b=>(
