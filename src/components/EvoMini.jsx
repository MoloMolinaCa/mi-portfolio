...
export default function EvoMini({en,trades,fxRate,liveT10Y,liveFX,liveSP500,historicos,isModal=false,livePricesAll={},onExpand=null,xirrFull=null,totPnlTotal=null}){
  ...
  const xirrData=useMemo(()=>{
    ...
    try{
      ...
      if(cd.spy100&&cd.spy100.length>=2){const spy100=cd.spy100;const spyEnd=spy100[spy100.length-1].val;const spyAt=(dateStr)=>{let best=spy100[0];for(const p of spy100){if(p.date<=dateStr)best=p;else break;}return best.val||100;};if(spyEnd>0){const spyFlows=[];spyFlows.push({date:s,amount:-startValUSD});for(const fl of flows.slice(1,-1))spyFlows.push({...fl});let spyFinalVal=0;for(const fl of spyFlows){const spyAtFlow=spyAt(fl.date);const growth=spyAtFlow>0?spyEnd/spyAtFlow:1;spyFinalVal+=(-fl.amount)*growth;}// Solo cobrarle al SPY comisiones sobre capital NUEVO (no sobre rotaciones)
// Para cada día: nuevo_capital = max(0, compras - ventas); comisión proporcional a esa fracción
const _netByDate={};for(const t of periodTrades){const T=String(t.ticker||'').toUpperCase();const isBond=isBondTicker(T);const raw=(+t.qty||0)*(+t.price||0)*(isBond?0.01:1);const com=+t.comision||0;const isUSD=(t.currency||'ARS')==='USD';const fx=isUSD?1:_getCCLForDate(t.date);if(!_netByDate[t.date])_netByDate[t.date]={buyUSD:0,sellUSD:0,buyComUSD:0};if(t.tipo==="compra"){_netByDate[t.date].buyUSD+=raw/fx;_netByDate[t.date].buyComUSD+=com/fx;}else{_netByDate[t.date].sellUSD+=raw/fx;}}
const _spyComAdj=Object.entries(_netByDate).reduce((a,[date,d])=>{const newCap=Math.max(0,d.buyUSD-d.sellUSD);if(!newCap||!d.buyUSD||!d.buyComUSD)return a;const comForNew=d.buyComUSD*(newCap/d.buyUSD);const g=spyAt(date)>0?spyEnd/spyAt(date):1;return a+comForNew*g;},0);spyFinalVal-=_spyComAdj;spDollarPnL=spyFinalVal-startValUSD+middleFlowsSum;spyFlows.push({date:e,amount:spyFinalVal});spyFlows.sort((a,b)=>a.date.localeCompare(b.date));if(spyFlows.length>=2&&startValUSD>0&&spyFinalVal>0){const rAnualSpy=calcXIRR(spyFlows);if(rAnualSpy!=null)spyXIRR=deannualizeXIRR(rAnualSpy,days)*100;}}}
